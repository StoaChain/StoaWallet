// Production-bundle derive->sign EXECUTION runner for the mobile app.
//
...
// Capacitor WebView presents — and reports the EXECUTED result.
//
...
// BEFORE the bundle (and therefore its polyfill) runs. A node+jsdom loader
// that left Node's native Buffer in place would MASK the precise
...
async function buildHarnessBundle(appRoot) {
  ...
  const output = await build({
    root: appRoot,
    // Do NOT load apps/mobile/vite.config.ts — it sets a SPA `build.outDir` and
    // registers `@vitejs/plugin-react`, neither of which belongs in this
    // standalone IIFE lib build. This harness is intentionally self-contained
    // (its own resolve/define), mirroring the extension harness fix.
    configFile: false,
    logLevel: 'error',
